...
class license_plate_detector:
    ...
    def __post_init__(self):
        ...
        self.model = YOLO(self.license_plate_model_path)
        ...
        print(f"License plate detector initialized on {device}")
        print(f"Batch size: {self.batch_size}")

    def process_frame(self, frame: np.ndarray, frame_number: int = 0) -> list:
        """
        Process a single frame and return license plate detections
        ...
        try:
            while True:
                ...
                if frame_count % 100 == 0:
                    print(f"Processed {frame_count} frames, {len(all_detections)} detections found")

        finally:
            cap.release()

        print(f"Total processed: {frame_count} frames, {len(all_detections)} license plates detected")
